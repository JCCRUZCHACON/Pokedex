...
const Pagination = ({
  ...
  return (
    <ul className="flex justify-center gap-4 p-4 items-center cursor-pointer">
      {currentPage >= 2 && <li onClick={handleFirstPage}>{"<<"}</li>}
      {currentPage >= 2 && <li onClick={handlePreviusPage}>{"<"}</li>}
      {pagesInCurrentBlock.map((page) => (
        <li className={`p-2 ${currentPage===page ? "text-white bg-red-500 rounded-full font-bold" : ""}`}  key={page} onClick={() => setCurrentPage(page)}>
          {page}
        </li>
      ))}
